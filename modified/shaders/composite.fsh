...
const float 	ambientOcclusionLevel 	= 0.3f;
...
float CalculateHeldLightShading(in SurfaceStruct surface)
{
	...
	float NdotL = dot(lightVector, surface.normal.xyz);
...
void 	AddSkyGradient(inout SurfaceStruct surface) {
	...
	surface.sky.albedo.g *= skyGradientFactor * 3.0f + 1.0f;
...
void 	CalculateAtmosphericScattering(inout vec3 color, in SurfaceStruct surface)
{
	...
	float redshift = 1.5f * (1.0f - rainStrength);
		  redshift *= float(surface.mask.land);

	//scatter away high frequency light
	color.b *= 1.0f - clamp(fogFactor * 1.65 * redshift, 0.0f, 0.75f);
	color.g *= 1.0f - fogFactor * 0.4* redshift;
...
vec4 CloudColor2(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector, in float altitude, in float thickness, const bool isShadowPass)
{
	...
	float t = frameTimeCounter * 1.0f;
	 ...
	 p += (Get3DNoise(p * 2.0f + vec3(0.0f, t * 0.01f, 0.0f)) * 2.0f - 1.0f) * 0.13f;
	...
	vec3 p1 = p * vec3(1.0f, 0.5f, 1.0f)  + vec3(0.0f, t * 0.01f, 0.0f);
	float noise  = 	Get3DNoise(p * vec3(1.0f, 0.5f, 1.0f) + vec3(0.0f, t * 0.01f, 0.0f));	p *= 2.0f;	p.x -= t * 0.017f;	p.z += noise * 1.35f;	p.x += noise * 0.25f; 									vec3 p2 = p;
		  noise += (2.0f - abs(Get3DNoise(p) * 2.0f - 0.0f)) * (0.25f);						p *= 3.0f;	p.xz -= t * 0.005f;	p.z += noise * 1.35f;	p.x += noise * 0.25f; 	p.x *= 3.0f;	p.z *= 0.55f;	vec3 p3 = p;
	...
	float coverage = 0.65f + rainy * 0.35f;
		  ...
		  float dist = length(worldPosition.xz - cameraPosition.xz);
		  coverage *= max(0.0f, 1.0f - dist / 10000.0f);
	float density = 0.0f;
	...
	else
	{
		...
		directLightFalloff *= 1.0f - rainStrength * 0.5f;
...
void main() {
	...
	surface.albedo = mix(surface.albedo, vec3(dot(surface.albedo, vec3(0.3333f))), vec3(0.025f));
	...
	lightmap.skylight 			*= mix(colorSkylight, colorBouncedSunlight, vec3(max(0.2f, (1.0f - pow(mcLightmap.sky + 0.13f, 1.0f) * 1.0f)))) + colorBouncedSunlight * (mix(1.3f, 2.8f, wetness)) * (1.0f - rainStrength);
	...
	final.glow.glowstone 			= Glowmap(surface.albedo, surface.mask.glowstone, 1.6f, colorTorchlight);
	...
	//Apply lightmaps to albedo and generate final shaded surface
	vec3 finalComposite = final.sunlight 			* 0.35f 	* sunlightMult				//Add direct sunlight
						+ final.skylight 			* 0.047f				//Add ambient skylight
						+ final.nolight 			* 0.00005f 			//Add base ambient light
						+ final.bouncedSunlight 	* 0.005f 	* sunlightMult				//Add fake bounced sunlight
						+ final.scatteredSunlight 	* 0.02f							//Add fake scattered sunlight
						+ final.scatteredUpLight 	* 0.0015f 	* sunlightMult
						+ final.torchlight 			* 1.0f 			//Add light coming from emissive blocks
						+ final.glow.lava			* 8.6f 			
						+ final.glow.glowstone		* 1.1f 			
						+ final.glow.fire			* 0.025f 			
						+ final.glow.torch			* 1.15f 	
						+ final.heldLight 			* 0.05f		* 0.0f
						;
	...
	gl_FragData[1] = vec4(surface.mask.matIDs, surface.shadow * surface.cloudShadow, mcLightmap.sky, 1.0f);
