...
#define NORMAL_MAP_MAX_ANGLE 2.0f   		//The higher the value, the more extreme per-pixel normal mapping (bump mapping) will be.
...
vec2 CalculateShadowCoord(in vec2 coord, in vec3 viewVector)
{
	vec2 parallaxCoord = coord.st;
	const int maxSteps = 1122;
	const vec3 stepSize = vec3(0.001f, 0.001f, 0.1f);
	...
	//{
		...
		//viewVector.xy = - viewVector.xy;

		if (heightmap < 1.0f)
		{
			vec3 step = viewVector * stepSize * 0.25f;

			float sampleHeight = texture2D(normals, OffsetCoord(coord.st, pCoord.st, 0)).a;

			for (int i = 0; sampleHeight < pCoord.z && i < 64; ++i)
			{
				//if (heightmap < pCoord.z)
				pCoord = mix(pCoord, pCoord + step, clamp((pCoord.z - sampleHeight) / (stepSize.z * 1.0 / (-viewVector.z + 0.01)), 0.0, 1.0));
				sampleHeight = texture2D(normals, OffsetCoord(coord.st, pCoord.st, 0)).a;
				//pCoord += step;
...
void main() {	
	...
	float height = texture2D(normals, parallaxCoord).a;
	...
	vec4 spec = texture2D(specular, parallaxCoord.st);
	...
	if (distance < bump_distance) {
			...
			vec3 bump = texture2D(normals, parallaxCoord.st, 0).rgb * 2.0f - 1.0f;
			
			float bumpmult = clamp(bump_distance * fademult - distance * fademult, 0.0f, 1.0f) * NORMAL_MAP_MAX_ANGLE;
	              bumpmult *= 1.0f - (max(0.0f, spec.g * 3.0f - 1.5f) * 0.75f);
