...
float GetWaves(vec3 position, in float scale) {
	...
	//p += textureSmooth(noisetex, (position.xz / 200.0f) - vec2(frameTimeCounter / 100.0f, 0.0f)).xy / 15.0f;
	...
	// weight = 12.25f;	
	// weights += weight;	
	// 	  wave = (textureSmooth(noisetex, (p * vec2(1.0f, 0.4f))  + vec2(0.0f,  p.x * 1.1f) ).x) * 0.5f;		p /= 1.2f; 	p.x += (frameTimeCounter / 55.0f) * speed;
